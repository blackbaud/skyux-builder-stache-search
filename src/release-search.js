...
function releaseSearch(argv, config) {
  ...
  if (!argv.audienceId) {
    return errorHandler(new Error('[ERROR]: An audienceId is required to release stache search data!'), config);
  }

  if (!argv.clientUserName || !argv.clientKey) {
    return errorHandler(new Error('[ERROR]: Client User Name and Client Key are required to release stache search data!'), config);
  }

  if (!argv.buildVersion  || !argv.siteName) {
    return errorHandler(new Error('[ERROR]: A build version and a site name are required to release stache search data!'), config);
  }
